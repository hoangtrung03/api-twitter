...
// Do you want use async await in handler function then use try catch
// If you don't want use async await in handler function then use wrapRequestHandler
export const audienceValidator = wrapRequestHandler(async (req: Request, res: Response, next: NextFunction) => {
  const tweet = req.tweet as Tweet

  if (tweet.audience === TweetAudience.TwitterCircle) {
    // Check user view tweet is logged in user
    if (!req.decoded_authorization) {
      throw new ErrorWithStatus({ status: HTTP_STATUS.UNAUTHORIZED, message: USER_MESSAGES.ACCESS_TOKEN_IS_REQUIRED })
    }

    // Check if the tweet author's account has been deleted or locked
    const author = await databaseService.users.findOne({ _id: new ObjectId(tweet.user_id) })

    if (!author || author.verify === UserVerifyStatus.Banned) {
      throw new ErrorWithStatus({ status: HTTP_STATUS.FORBIDDEN, message: USER_MESSAGES.USER_NOT_FOUND })
    }

    // Check if this tweet is in the author's tweet circle
